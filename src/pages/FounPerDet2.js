...
function FounPerDet2() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      const formData = {
        Qualifications: latestEducation,
      ...
      try {
        const response = await axios.patch(
            `http://localhost:4000/api/FounderDet`,
            formData,{
              headers: {
        ...
        if (response.status === 200) {
            console.log(" Founder Details-2 updated:", response.data.message);
            navigate("/Achieve"); // Navigate to home or any other page
        } else {
