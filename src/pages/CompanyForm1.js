    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (response.status === 201) {
          ...
          navigate("/CompanyForm2"); // Pass to the next form
      } else {
        ...
        <div className="bg-white p-8 rounded shadow-md sm:max-w-md w-full">
          ...
          <form className="mt-6" onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              ...
              <select
                id="domain"
                className={` text-black mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-slate-700 focus:border-slate-700 bg-white sm:text-sm ${domain ? 'text-black' : ''}`}
                value={domain}
